...
public class AiCheckMedia {
    ...
    public void checkMedia(AbsSender sender, Update update) throws TelegramApiException {
        ...
        if (groupInfoWithBLOBs != null && "open".equals(groupInfoWithBLOBs.getAiflag())) {
            ...
            } else if (update.getMessage().hasVideo() && update.getMessage().getVideo().getThumbnail() != null) {
                fileId = update.getMessage().getVideo().getThumbnail().getFileId();
            } else if (update.getMessage().hasVideoNote() && update.getMessage().getVideoNote().getThumbnail() != null) {
            ...
            if (StringUtils.hasText(fileId)) {
                ...
                if (file != null) {
                    log.info("媒体AI检测：Telegram文件下载成功，groupId={}, userId={}, messageId={}", groupId, userId, messageId);
                    String miaoshu = "";
                    List<EntityAnnotation> list = googleCloudVisionService.detectTextFromLocalImage(file);
                    if (!list.isEmpty()) {
                        miaoshu = list.get(0).getDescription();
                    }
                    log.info("媒体AI检测：Google Vision OCR完成，groupId={}, userId={}, messageId={}, textLength={}", groupId, userId, messageId, miaoshu.length());
                    SafeSearchAnnotation safeSearchAnnotation = googleCloudVisionService.detectSafeSearchFromLocalImage(file);
                    log.info("媒体AI检测：Google Vision SafeSearch完成，groupId={}, userId={}, messageId={}, hasResult={}", groupId, userId, messageId, safeSearchAnnotation != null);
